...
static metadb_handle_ptr g_tf_current_track;
...
void titleformat_provider::clear_track_artwork_info() {
    metadb_handle_ptr old_track;
    {
        std::lock_guard<std::mutex> lock(g_tf_mutex);
        old_track = g_tf_current_track;
        g_tf_current_track.release();
        g_tf_artist.reset();
        g_tf_title.reset();
        g_tf_cover.reset();
        g_tf_source.reset();
    ...
    if (old_track.is_valid() && core_api::is_main_thread()) {
        try {
            metadb_io::get()->dispatch_refresh(old_track);
        } catch (...) {}
    }
}
...
public:
    ...
    bool process_field(t_uint32 index, metadb_handle* handle, titleformat_text_out* out) override {
        pfc::string8 current_artist, current_title, current_cover, current_source;
        metadb_handle_ptr current_track;
        {
            ...
            current_track = g_tf_current_track;
        }

        pfc::string8 field_val;
        bool is_current_track = false;

        if (handle != nullptr && current_track.is_valid()) {
            if (handle == current_track.get_ptr() || strcmp(handle->get_path(), current_track->get_path()) == 0) {
                is_current_track = true;
            }
        } else if (handle == nullptr || !current_track.is_valid()) {
            is_current_track = true;
        }
        ...
        if (field_val.is_empty() && handle != nullptr) {
            try {
