...
std::string MetadataCleaner::extract_primary_title(const char* title) {
    ...
    if (res.length() >= 2 && ((res.front() == '"' && res.back() == '"') || (res.front() == '\'' && res.back() == '\''))) {
        res = trim(res.substr(1, res.length() - 2));
    }
    return res;
}
...
bool MetadataCleaner::is_youtube_topic_artist(const char* artist) {
    std::string name = trim(artist ? artist : "");
    to_lower_ascii(name);
    const std::string suffix = "- topic";
    return name.size() > suffix.size() &&
           name.compare(name.size() - suffix.size(), suffix.size(), suffix) == 0;
}

bool MetadataCleaner::try_parse_youtube_description(const char* desc, std::string& out_artist, std::string& out_title, std::string& out_album) {
    out_artist.clear();
    out_title.clear();
    out_album.clear();
    ...
    // Split desc into non-empty trimmed lines
    ...
    // Detect YouTube Music Art Track format:
    // Line 1 typically begins with "Provided to YouTube by" (case-insensitive)
    // Or Line 2 contains middle dot '·' (\xC2\xB7) while Line 1 has "YouTube"
    std::string lower_line0 = lines[0];
    to_lower_ascii(lower_line0);

    bool has_provided_by = (lower_line0.find("provided to youtube") != std::string::npos);

    // Check line 2 for middle dot '·' (U+00B7, UTF-8: 0xC2 0xB7) or bullet '•' (\xE2\x80\xA2)
    size_t dot_pos = lines[1].find("\xC2\xB7");
    size_t dot_len = 2;
    if (dot_pos == std::string::npos) {
        dot_pos = lines[1].find("\xE2\x80\xA2"); // bullet '•'
        dot_len = 3;
    }

    if (dot_pos == std::string::npos && has_provided_by) {
        dot_pos = lines[1].find(" - ");
        dot_len = 3;
    }

    if (!has_provided_by && dot_pos == std::string::npos) {
        return false;
    }
    if (dot_pos == std::string::npos) {
        return false;
    ...
    // Line 2: Title · Artist
    std::string tit = trim(lines[1].substr(0, dot_pos));
    std::string art = trim(lines[1].substr(dot_pos + dot_len));

    if (tit.empty() || art.empty()) return false;

    out_title = tit;
    // Art Tracks can list several artists separated by the same middle dot.
    // Reuse the collaboration handling instead of treating that list as one name.
    for (const std::string& separator : {std::string("\xC2\xB7"), std::string("\xE2\x80\xA2")}) {
        size_t pos = 0;
        while ((pos = art.find(separator, pos)) != std::string::npos) {
            art.replace(pos, separator.size(), " & ");
            pos += 3;
        }
    }
    out_artist = normalize_whitespace(art);

    // Line 3: Album (if available and not copyright/metadata line)
    if (lines.size() >= 3) {
        const std::string& line2 = lines[2];
        std::string lower_line2 = line2;
        to_lower_ascii(lower_line2);

        bool is_copyright = (line2.find("\xE2\x84\x97") != std::string::npos) || // ℗
                            (line2.find("(P)") != std::string::npos) ||
                            (line2.find("(C)") != std::string::npos) ||
                            (line2.find("\xC2\xA9") != std::string::npos) || // ©
                            (lower_line2.find("released on:") != std::string::npos) ||
                            (lower_line2.find("auto-generated by youtube") != std::string::npos);

        if (!is_copyright) {
            out_album = trim(line2);
        } else {
            out_album = out_title;
        }
    } else {
...
bool MetadataCleaner::try_split_multipart_title(const std::string& title, const std::string& raw_artist, std::string& out_field1, std::string& out_field2, bool is_youtube) {
    out_field1.clear();
    out_field2.clear();
    if (title.empty()) return false;

    // Detect primary delimiter in title in priority order:
    // 1. Bullet " • " or "•" (\xE2\x80\xA2)
    // 2. Pipe " | " or "|"
    // 3. Tilde " ~ " or "~"
    // 4. Spaced dashes (" - ", " – ", " — ", etc.)
    // 5. Spaced slash " / "
    std::string chosen_delim;
    if (title.find("\xE2\x80\xA2") != std::string::npos) {
        chosen_delim = (title.find(" \xE2\x80\xA2 ") != std::string::npos) ? " \xE2\x80\xA2 " : "\xE2\x80\xA2";
    } else if (title.find(" | ") != std::string::npos || (title.find('|') != std::string::npos && title.find(" - ") == std::string::npos)) {
        chosen_delim = (title.find(" | ") != std::string::npos) ? " | " : "|";
    } else if (title.find(" ~ ") != std::string::npos || (title.find('~') != std::string::npos && title.find(" - ") == std::string::npos)) {
        chosen_delim = (title.find(" ~ ") != std::string::npos) ? " ~ " : "~";
    } else {
        static const std::vector<std::string> dash_delims = {
            " - ",
            " \xE2\x80\x93 ", // en-dash " – "
            " \xE2\x80\x94 ", // em-dash " — "
            " \xCB\x97 ",     // modifier letter minus " ˗ "
        ...
        for (const auto& d : dash_delims) {
            if (title.find(d) != std::string::npos) {
                chosen_delim = d;
                break;
            }
        }
    ...
    if (chosen_delim.empty()) return false;

    // Split title by chosen_delim into non-empty trimmed segments
    std::vector<std::string> segments;
    size_t start = 0;
    while (start < title.length()) {
        size_t pos = title.find(chosen_delim, start);
        if (pos == std::string::npos) {
            std::string seg = trim(title.substr(start));
            if (!seg.empty()) segments.push_back(seg);
            break;
        } else {
            std::string seg = trim(title.substr(start, pos - start));
            if (!seg.empty()) segments.push_back(seg);
            start = pos + chosen_delim.length();
        }
    }

    if (segments.size() < 2) return false;

    // Keep the quoted song name when followed by a performance credit.
    // Example: Alcione | "Poder da Criacao" com Diogo Nogueira | ...
    auto clean_segment = [](const std::string& segment) {
        std::string text = normalize_quotes_and_apostrophes(segment);
        static const std::regex quoted_credit(
            "^\\s*[\"'](.+?)[\"']\\s+(?:com|with|feat\\.?|ft\\.?|featuring)\\s+.+$",
            std::regex_constants::icase);
        std::smatch match;
        if (std::regex_match(text, match, quoted_credit)) text = match[1].str();
        return clean_for_search(text.c_str(), true);
    };
    std::string f1 = clean_segment(segments[0]);
    std::string f2 = clean_segment(segments[1]);

    // Strip enclosing quotes if present
    if (f1.length() >= 2 && ((f1.front() == '"' && f1.back() == '"') || (f1.front() == '\'' && f1.back() == '\''))) {
        f1 = trim(f1.substr(1, f1.length() - 2));
    }
    if (f2.length() >= 2 && ((f2.front() == '"' && f2.back() == '"') || (f2.front() == '\'' && f2.back() == '\''))) {
        f2 = trim(f2.substr(1, f2.length() - 2));
    }
...
StreamMetadataResult MetadataCleaner::sanitize_stream_metadata(const char* raw_artist, const char* raw_title, bool is_youtube, bool known_track_metadata) {
    ...
    if (lower_art_suffix.length() > 8 && lower_art_suffix.compare(lower_art_suffix.length() - 8, 8, " - topic") == 0) {
        res.clean_artist = trim(res.clean_artist.substr(0, res.clean_artist.length() - 8));
    } else if (lower_art_suffix.length() > 7 && lower_art_suffix.compare(lower_art_suffix.length() - 7, 7, "- topic") == 0) {
        res.clean_artist = trim(res.clean_artist.substr(0, res.clean_artist.length() - 7));
    }
    ...
    if (!split_done && is_youtube && !res.raw_title.empty()) {
        std::string f1, f2;
    ...
    // 2. Untagged radio stream or local track: if artist is placeholder, try multi-part title split
    if (!split_done && artist_is_placeholder && !res.raw_title.empty()) {
        std::string f1, f2;
    ...
    // 3. Fallback to existing try_split_combined if still placeholder
    if (!split_done) {
